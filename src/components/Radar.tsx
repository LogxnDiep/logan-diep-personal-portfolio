...
export default function Radar({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ alpha: true, premultipliedAlpha: false } as any);
    ...
    const resize = () => {
      ...
      if (program) {
        (program.uniforms as Record<string, { value: number[] }>).uResolution.value = [
          gl.canvas.width,
          gl.canvas.height,
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColor: { value: new Float32Array(hexToVec3(color)) },
        uBgColor: { value: new Float32Array(hexToVec3(backgroundColor)) },
    ...
    const update = (time: number) => {
      ...
      (program.uniforms as Record<string, { value: number }>).uTime.value = time * 0.001;

      if (enableMouseInteraction) {
        currentMouse[0] += 0.05 * (targetMouse[0] - currentMouse[0]);
        currentMouse[1] += 0.05 * (targetMouse[1] - currentMouse[1]);
        ((program.uniforms as Record<string, { value: Float32Array }>).uMouse.value as Float32Array)[0] = currentMouse[0];
        ((program.uniforms as Record<string, { value: Float32Array }>).uMouse.value as Float32Array)[1] = currentMouse[1];
      } else {
        ((program.uniforms as Record<string, { value: Float32Array }>).uMouse.value as Float32Array)[0] = 0.5;
        ((program.uniforms as Record<string, { value: Float32Array }>).uMouse.value as Float32Array)[1] = 0.5;
      }
